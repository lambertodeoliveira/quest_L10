        ...
        if (! empty($builder->columns)) {
            ...
            foreach ($builder->columns as $as => $column) {
                if ($column instanceof Expression) {
                    if (stripos($column->getValue($builder->grammar), 'AS fuzzy_relevance_')) {
                        $matches = [];

                        preg_match('/AS (fuzzy_relevance_.*)$/', $column->getValue($builder->grammar), $matches);

                        if (! empty($matches[1])) {
                            $existingRelevanceColumns[$as] = $matches[1];
                        }
                    } elseif (stripos($column->getValue($builder->grammar), 'AS _fuzzy_relevance_')) {
                        $sumColumnIdx = $as;
                    }
                }
            }
            ...
            foreach ($existingRelevanceColumns as $as => $column) {
                $relevanceTotalColumn .= (! empty($relevanceTotalColumn) ? ' + ' : '')
                    . '('
                    . str_ireplace(' AS ' . $column, '', $builder->columns[$as]->getValue($builder->grammar))
                    . ')';
            }
